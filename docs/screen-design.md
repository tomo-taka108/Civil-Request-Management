...
| 2026-07-20 | 0.3 | 地図表示画面のモックアップ（`mockup/map.html`）を新規作成し、画面一覧（1章）の対応モックアップを更新。あわせてレビューで判明した検索仕様の変更（要件定義書2.3：対応部署・対応状況・緊急性を単一選択から複数選択に変更）を6章の残課題に追記 | - |
...
> #8・#10 はモックアップ未作成。本設計書で機能要件（2.5章）から存在が必須と判断し、次回のモックアップ拡充対象とする。#7（地図表示）は本改訂で`map.html`を作成済み。
...
| MapController | index / pins | `map/index.blade.php` | `requests`から緯度経度を持つ案件を条件検索（事務所スコープ適用）。`pins`は一覧の各件の緯度経度・受付日時・緊急性等をGeoJSON等で返す |
