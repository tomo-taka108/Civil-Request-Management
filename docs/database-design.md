...
- [x] 初回のシステム管理者アカウント作成方法 → `AdminUserSeeder` で1件作成する（`user_id=admin`・初期パスワードは投入前に変更、`must_change_password=true`）。`DatabaseSeeder` に登録済み
- [x] `offices` マスタの初期データ（事務所一覧）の投入方法 → `OfficeSeeder` で投入する。`DatabaseSeeder` に登録済み（冪等）

**今回のスコープ外（運用開始後・必要時に検討）**

- [ ] MySQLのFULLTEXT日本語全文検索の精度検証（Ngramパーサ設定等）。全文検索インデックスは実装済み。日本語での検索精度は運用データで確認し、不十分なら`LIKE`検索への切り替えを検討する
- [ ] 地図上の範囲指定検索（緯度経度の範囲指定 or 空間インデックス導入）。住所・各種条件での絞り込みで運用要件は満たせるため、必要になった時点で追加する（要件定義書 2.3）
- [ ] データ保存期間3年経過後の廃棄処理（バッチ削除）の実装方式。運用開始後、実際の保存データが3年に達する前に整備すればよい
