...
| 2026-07-20 | 0.5 | 追加の第三者レビュー指摘を反映：(1) ALBの既存Listenerもimport不要と判断し、`data "aws_lb_listener"`参照＋`aws_lb_listener_rule`追加のみに変更（4.1・4.2、importスコープを実質ゼロに縮小）、(2) RDS権限からMySQL 8で実質不要なREFERENCESを削除（3.1）、(3) Dockerネットワーク分離の理由を「名前解決の防止」から「コンテナ間通信の必要最小限化」に訂正（同一ホストではlocalhost経由の到達は防げないため、3.3）、(4) EC2相乗りにDocker Volumeの独立方針を追記（3.2）、(5) EC2追加コストの表現を「なし」から「現時点では追加料金なし、リソース逼迫時はスケールアップ費用の可能性あり」に修正（3.7）、(6) tfstate関連の手順記載先をREADMEからTerraform運用手順書に変更（4.4） | - |
| 2026-07-20 | 0.6 | 誤りを修正：(1) 「スキーマ管理はLaravel Migrationで行う」という記述がCREATE DATABASE/USER/GRANTまで含むかのように誤読される表現だったため、データベース・ユーザー作成（RDS上で事前実施、Terraform・Laravel Migrationいずれの管轄でもない）とテーブル・インデックス等のスキーマ管理（Laravel Migrationの管轄）を明確に分離（3.1・4.3）、(2) `terraform plan`確認の表現を「追加差分のみであることを確認」から「表示された差分の内容が意図と一致しているかを確認」に修正。プロバイダー更新やドリフト、意図した変更による差分まで機械的に否定しないよう訂正（4.2） | - |
| 2026-07-20 | 0.7 | 第三者レビュー指摘を反映：「Terraform・Laravel Migrationいずれの管轄にも含めない」という記述だと責任の所在が不明瞭だったため、RDSデータベース・アプリケーションユーザー作成はRDSに対する初期セットアップ手順として実施・管理する旨を明記し、責任範囲を明確化（4.3） | - |
...
- 既存famigo環境は手動構築（ClickOps）されており、TerraformなどのIaC管理下にない。本システムのインフラはTerraformで管理するが、famigoの相乗り先リソース（VPC・Subnet・SG・ALB等、新規リソースが依存する部分）は原則`data source`で参照するに留め、本システムが新規に追加するリソースのみを`resource`として管理する（詳細は「4. Terraform化方針」）
- famigoとのリソース共存部分（EC2上のコンテナ実行環境、Dockerネットワーク）は、片方の変更・デプロイがもう片方に影響しないよう明確に分離する（「3.2 EC2相乗り」「3.3 Dockerネットワーク」参照）
...
- 権限は`GRANT ALL`ではなく、Laravel（Eloquent・Migration）が実際に必要とする範囲（DML：SELECT/INSERT/UPDATE/DELETE、DDL：CREATE/ALTER/INDEX/DROP）に絞った最小権限とする。REFERENCES権限はMySQL 8では外部キー制約の作成に実質不要なため付与しない
- インスタンス自体（CPU・メモリ・IOPS・自動バックアップ設定）はfamigoと共用のため、パラメータグループ変更やバックアップ保持期間の変更は行わない（要件定義書4.3参照）
- **データベース（`CREATE DATABASE`）・アプリケーションユーザー作成（`CREATE USER`/`GRANT`）はRDS上で事前に実施する作業であり、Laravel Migrationの管轄外**。Laravel Migrationが管理するのは、作成済みのデータベース内のテーブル・カラム・インデックス・外部キー制約のみ（いずれもTerraformの管轄外）
...
famigo用コンテナと本システム用コンテナで、**Dockerネットワークを分離する**（`famigo-network` / `civil-network`のように、docker-compose.yml側でネットワークを分ける）。同一ホスト上で動く以上、`localhost:8080`のようなホスト経由のアクセスまでは防げないが、コンテナ名による直接的な名前解決・到達性をなくすことで、コンテナ間通信を必要最小限に限定し、誤接続や設定ミスによる影響範囲を小さくするため。
...
| EC2アプリ追加 | 現時点では追加料金なし（同一インスタンス内）。ただしCPU・メモリ使用率が逼迫した場合、インスタンスタイプのスケールアップ（例：t3.micro→t3.small）が必要になり、その場合は差額が発生する可能性あり（「3.4 EC2のリソース検証」参照） |
...
- ALBの既存Listener（443）についても、importは不要と判断する。`aws_lb_listener_rule`は`listener_arn`（文字列）を引数に取るだけで、そのARNの発行元が`resource`か`data`かをTerraformは区別しない。そのため`data "aws_lb_listener"`で既存Listenerを参照し、そのARNを渡して`aws_lb_listener_rule`を追加すれば、Listener自体を管理下に置かなくても新規ルール追加は安全に行える
- 使い分けの基準：
  - **data source参照**（import不要）：VPC・Subnet・既存SG（`famigo-sg-ec2`等）・RDSインスタンス本体・EC2インスタンス本体・既存ALB本体・既存ALB Listener・既存Route53ホストゾーン（famigo側）など、参照はするが本システム側から変更を加えない既存リソース全般
  - **import**：本方針では対象なし（新規追加するものはすべて`resource`ブロックで素直に作成する）

### 4.2 手順

1. `data "aws_vpc"` `data "aws_subnet"` `data "aws_security_group"` `data "aws_lb"` `data "aws_lb_listener"` `data "aws_db_instance"` `data "aws_route53_zone"`（本システム用に新規取得するドメインのゾーンは`resource`側で新規作成）等で、依存する既存famigoリソースを参照する
2. `data "aws_lb_listener"`で取得したARNを`listener_arn`に渡し、本システム用の`aws_lb_listener_rule`を`resource`として追加する（importは行わない）
3. その他の本システム用新規リソース（SGルール、ターゲットグループ、Route53ホストゾーン・レコード、ACM証明書等）も`resource`として追加し、`terraform plan`の出力を確認した上で`apply`する。「追加差分のみ」と機械的に限定するのではなく、**表示された差分の内容（新規作成／変更／削除、対象リソース）が意図と一致しているかを都度確認する**（プロバイダー更新やAWS側のドリフト、意図した設定変更により、新規作成以外の差分が表示されることもあり得るため）

### 4.3 Terraformの管轄外とするもの

- RDS内のデータベース・アプリケーションユーザー作成（`CREATE DATABASE` / `CREATE USER` / `GRANT`）：Terraform・Laravel Migrationいずれの管理対象外とし、RDSに対する初期セットアップ手順として実施・管理する
- RDS内のテーブル・カラム・インデックス・外部キー制約：Laravel Migration側で管理
